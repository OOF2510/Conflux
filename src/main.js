...
async function createWindow() {
  ...
  if (store.get("options.bttv")) {
    await mainWindow.webContents.session
      .loadExtension(path.resolve(__dirname, ".") + "/exts/BTTV")
      .then(({ id }) => {
  ...
  Menu.setApplicationMenu(
    menu(store, global.services, mainWindow, app, defaultUserAgent)
  );
  ...
  if (relaunchToPage !== undefined) {
    ...
    if (defaultService.url) {
      ...
      mainWindow.webContents.userAgent = defaultService.userAgent
